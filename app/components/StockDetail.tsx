      ...
      <div className="panel">
        ...
        <div className="setup-box">
          ...
          <div className="setup-g4">
            <div className="setup-item">
              <div className="sik">{T('Entry','دخول')}</div>
              <div className="siv">{data.entry}</div>
            </div>
            <div className="setup-item">
              <div className="sik">{T('Stop Loss','وقف الخسارة')}</div>
              <div className="siv" style={{color:'#E85555'}}>{data.stopLoss}</div>
            </div>
            <div className="setup-item">
              <div className="sik">{T('Target 1','الهدف 1')}</div>
              <div className="siv" style={{color:'#2EC98A'}}>{data.target1}</div>
            </div>
            <div className="setup-item">
              <div className="sik">{T('Target 2','الهدف 2')}</div>
              <div className="siv" style={{color:'#2EC98A'}}>{data.target2}</div>
            </div>
          </div>
        ...
        <div className="dcard">
          <PriceChart
            ...
            ticker={data.ticker}
            s1={+(tc.support1||0)||undefined}
            r1={+(tc.resistance1||0)||undefined}
          />
        </div>
        ...
        <div className="dcard">
          ...
          {an.catalysts?.length>0&&(
            <div style={{marginTop:10,background:'var(--gold-bg)',borderRadius:8,padding:'9px 12px',border:'0.5px solid rgba(201,168,76,.2)'}}>
              <div style={{fontSize:10,color:'var(--gold)',textTransform:'uppercase',letterSpacing:'.5px',marginBottom:5}}>
                ⚡ {T('Catalysts','المحفزات')}
              </div>
              {an.catalysts.map((c,i)=>(
                <div key={i} style={{fontSize:12,color:'var(--t2)',marginBottom:3}}>· {c}</div>
              ))}
            </div>
          )}
        </div>
        ...
        <div className="dcard">
          ...
          <div style={{display:'flex',gap:20,alignItems:'center',flexWrap:'wrap'}}>
            <div style={{textAlign:'center'}}>
              <div style={{fontSize:22,fontWeight:500,fontFamily:'monospace',color:'#2EC98A'}}>{str(ar.buy)}</div>
              <div style={{fontSize:10,color:'var(--t3)'}}>{T('Buy','شراء')}</div>
            </div>
            <div style={{textAlign:'center'}}>
              <div style={{fontSize:22,fontWeight:500,fontFamily:'monospace',color:'#E8A630'}}>{str(ar.hold)}</div>
              <div style={{fontSize:10,color:'var(--t3)'}}>{T('Hold','احتفاظ')}</div>
            </div>
            <div style={{textAlign:'center'}}>
              <div style={{fontSize:22,fontWeight:500,fontFamily:'monospace',color:'#E85555'}}>{str(ar.sell)}</div>
              <div style={{fontSize:10,color:'var(--t3)'}}>{T('Sell','بيع')}</div>
            </div>
            <div style={{flex:1,minWidth:140}}>
              <div style={{fontSize:12,color:'var(--t2)'}}>
                {T('Avg Target','الهدف المتوسط')}: <span style={{color:'var(--gold)',fontFamily:'monospace'}}>{str(ar.avgTarget)}</span>
              </div>
              <div style={{fontSize:12,color:'var(--t2)',marginTop:3}}>
                {T('High','أعلى')}: <span style={{color:'#2EC98A',fontFamily:'monospace'}}>{str(ar.highTarget)}</span>
                {' · '}
                {T('Low','أدنى')}: <span style={{color:'#E85555',fontFamily:'monospace'}}>{str(ar.lowTarget)}</span>
              </div>
          ...
          {(data.news||[]).map((n,i)=>{
            ...
            return (
              <div key={i} className="nitem">
                ...
                <div className="nmeta">
                  <span className="nsrc">{n.source} · {n.time}</span>
                  <span className={`nbadge ${s.cls}`}>{lang==='ar'?s.ar:s.en}</span>
                </div>
              </div>
            )
          })}
        ...
        <div className="disc">
          {T('For informational purposes only. Not financial advice.','للأغراض التعليمية فقط. ليست نصيحة مالية.')}
        </div>
      </div>
    </div>
  )
